...
int hwid_hack()
{
	...
	NtQueryKey = (NTQK)GetProcAddress(GetModuleHandleW(L"ntdll.dll"), "NtQueryKey");
	if (!NtQueryKey)
	{
		printf("[-] 获取函数NtQueryKey地址失败 \n");
		return 1;
	}
	...
	SHGetFolderPathW(0, CSIDL_APPDATA, 0, SHGFP_TYPE_DEFAULT, appdata);
	SHGetFolderPathW(0, CSIDL_LOCAL_APPDATA, 0, SHGFP_TYPE_DEFAULT, localappdata);
						...
						if (0 == memcmp(sector + s->NameOffset, s->Name, strlen(s->Name)))
						{
							...
							*(PDWORD)(sector + s->SerialOffset) = (rand() << 16) + rand();
							if (INVALID_SET_FILE_POINTER != SetFilePointer(device, 0, 0, FILE_BEGIN))
								WriteFile(device, sector, sizeof(sector), &len, 0);
							printf("[+] 写入硬盘数据量  %d \n", len);
							break;
						}
					}
...
int main(int argc, char* argv[])
{
	...
	do { printf("按键q退出程序"); } while (getchar() != 'q');
	return 0;
}